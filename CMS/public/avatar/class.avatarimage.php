...
class AvatarImage
{
	var $version				= "1.0 powered by shadeCMS";
	var $error					= null;
	var $debug					= null;
	var $settings				= null;
	...
	{	
		...
		if($this->isHeadOnly)
		{
			$temp = imageCreateTrueColor($this->rectWidth-10, $this->rectHeight-48);
			imageAlphaBlending($temp, false);
			$rectMask = imageColorAllocateAlpha($temp, 255, 0, 255, 127);
			imageSaveAlpha($temp, true);
			$x = ImageCopyResized($temp, $avatarImage, -6, -7, 0, 0, $this->rectWidth, $this->rectHeight, $this->rectWidth, $this->rectHeight);
			if($x) {
				$avatarImage = $temp;
			}
		}
	...
	function getPartResource($uniqueName, $action, $type, $partId, $direction)
	{
		...
		if($uniqueName == "acc_chest_U_pg2sachb");
